...
from .serializers import ClickTransactionSerializer
...
class PyClickMerchantAPIView(APIView):
    ...
    def click_webhook_errors(cls, click_trans_id: str,
        ...
        if order.status == ClickOrder.CONFIRMED:
            return {
                'error': ALREADY_PAID,
        ...
        if order.status == ClickOrder.CANCELED or int(error) < 0:
            return {
    ...
    def prepare(cls, click_trans_id: str,
        ...
        order = cls.order_load(merchant_trans_id)
        if result['error'] == '0':
            order.status = ClickOrder.WAITING
            order.save()
    ...
    def complete(cls, click_trans_id: str,
        ...
        if error and int(error) < 0:
            order.change_status(ClickOrder.CANCELED)
        if result['error'] == SUCCESS:
            order.change_status(ClickOrder.CONFIRMED)
