...
class ApiHelper:
    ...
    @classmethod
    def get_extra_data(cls, transaction: ClickOrder):
        extra_data = {}
        try:
    ...
    @classmethod
    def save_extra_data(cls, transaction: ClickOrder, extra_data: dict):
    ...
    @classmethod
    def get_transaction(cls, transaction_id: int) -> Union[ClickOrder, dict]:
        try:
            transaction = ClickOrder.objects.get(id=int(transaction_id))
            return transaction
    ...
    def create_invoice(self):
        ...
        if isinstance(transaction, ClickOrder):
            ...
            if invoice.status_code == 200:
                ...
                if _json['error_code'] == SUCCESS:
                    transaction.change_status(ClickOrder.PROCESSING)
                else:
                    transaction.change_status(ClickOrder.ERROR)
    ...
    def check_invoice(self):
        ...
        if isinstance(transaction, ClickOrder):
            ...
            if check_invoice.status_code == 200:
                ...
                if _json['status'] > 0:
                    transaction.change_status(ClickOrder.CONFIRMED)
                elif _json['status'] == -99:
                    transaction.change_status(ClickOrder.CANCELED)
                elif _json['status'] < 0:
                    transaction.change_status(ClickOrder.ERROR)
                transaction.message = json.dumps(_json)
                transaction.save()
    ...
    def create_card_token(self):
        transaction = self.get_transaction(self.transaction_id)
        if isinstance(transaction, ClickOrder):
            if transaction.status == ClickOrder.CONFIRMED:
                return {
                    'error': ALREADY_PAID,
    ...
    def verify_card_token(self):
        transaction = self.get_transaction(self.transaction_id)
        if isinstance(transaction, ClickOrder):
            if transaction.status != ClickOrder.PROCESSING:
                return {
                    'error': -5001,
