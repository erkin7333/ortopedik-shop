...
class Cart(object):
    ...
    def __iter__(self):
        ...
        for item in self.cart.values():
            ...
            item['vendor_code'] = str(product.vendor_code)
    ...
    def add(self, product_id, quantity=1, update_quantity=False, color=None, size=None, vendor_code=None, price=None):
        product_id = str(product_id)

        if product_id not in self.cart:
            self.cart[product_id] = {'quantity': int(quantity), 'id': product_id,
                                     'color': color, 'size': size,
                                     "price": price, 'vendor_code': vendor_code}
            self.save()
        if update_quantity:
